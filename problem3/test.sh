...
python3 aws_inspector.py --region us-east-1 --format json > /dev/null
if [ $? -eq 0 ]; then
...
python3 aws_inspector.py --region us-east-1 --format json --output test_output.json
...
python3 aws_inspector.py --region us-east-1 --format table | head -10
echo "✓ Table format displayed"

# Test 4: Invalid region handling
echo "Test 4: Invalid region handling"
python3 aws_inspector.py --region invalid-region 2>/dev/null
if [ $? -ne 0 ]; then
