...
curl -s http://localhost:8081/papers | python3 -m json.tool > /dev/null
if [ $? -eq 0 ]; then
...
curl -s http://localhost:8081/stats | python3 -m json.tool > /dev/null
if [ $? -eq 0 ]; then
...
curl -s "http://localhost:8081/search?q=machine" | python3 -m json.tool > /dev/null
if [ $? -eq 0 ]; then
